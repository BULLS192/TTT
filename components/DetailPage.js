...
const profiles = {
  audio: {
    introTitle: 'Keep the factory controls. Fix the signal path. Tune the cabin.',
    facts: [['START WITH','Factory source and amplifier'],['KEEP WORKING','Screen, steering controls, chimes'],['MEASURE','Signal behavior and cabin response'],['FINISH WITH','A tuned system, not a pile of parts']],
    visual: 'signal'
  },
  'window-tint': {
    introTitle: 'Choose film for the way the car is driven, not just how dark it looks.',
    facts: [['DECIDE FIRST','Heat, privacy or appearance'],['CHECK','Factory glass and current VLT'],['PROTECT','Night visibility and optical clarity'],['VERIFY','Product specs and local limits']],
    visual: 'tint'
  },
  security: {
    introTitle: 'Security works better in layers than it does as one impressive box.',
    facts: [['RISK','Where and how the vehicle is parked'],['LAYERS','Detect, alert, immobilize, locate'],['DAILY USE','Protection cannot become a nuisance'],['SENSITIVE DETAILS','Installation specifics stay private']],
    visual: 'security'
  },
  tracking: {
    introTitle: 'A tracker for one owner and telematics for a fleet solve different problems.',
    facts: [['WHO NEEDS IT','Owner, dealer or fleet team'],['DATA','Location, zones, history, events'],['SERVICE','Cellular plan and platform access'],['LIFECYCLE','Activation, replacement and admin']],
    visual: 'tracking'
  },
  cameras: {
    introTitle: 'Start with the event you need to see, then design the coverage.',
    facts: [['COVERAGE','Front, rear, cabin or 360'],['PARKING','Power strategy matters'],['STORAGE','Local card or connected platform'],['RETRIEVAL','Footage must be easy to access']],
    visual: 'cameras'
  },
  lighting: {
    introTitle: 'Good lighting has a job to do.',
    facts: [['PURPOSE','Ambient, task or exterior utility'],['CONTROL','Factory trigger, switch or controller'],['POWER','Circuit load and protection'],['FINISH','No exposed add-on look']],
    visual: 'generic'
  },
  electronics: {
    introTitle: 'Accessories are easy. Clean electrical integration is the actual job.',
    facts: [['FUNCTION','What needs power or control'],['INTERFACE','How the driver uses it'],['ELECTRICAL','Load, circuit protection, sleep behavior'],['SERVICE','Accessible and understandable later']],
    visual: 'generic'
  },
  'custom-fabrication': {
    introTitle: 'When the right part does not exist, the mount becomes part of the engineering.',
    facts: [['FIT','Space, clearances and service access'],['MATERIAL','Structure, heat and finish'],['VEHICLE','Use existing geometry intelligently'],['RESULT','Looks intentional and can be serviced']],
    visual: 'generic'
  }
};
...
function TechnicalVisual({ item, kind }) {
  ...
  if (visual === 'signal') return <div className="technical-flow">
    {[['01','Factory source'],['02','Signal access'],['03','DSP / control'],['04','Amplification'],['05','Speakers + sub']].map(([n,title],i)=><div className="flow-node" key={title}><small>{n}</small><strong>{title}</strong>{i<4?<em>→</em>:null}</div>)}
  </div>;
  ...
  if (visual === 'tint') return <div className="comparison-grid">
    ...
    ].map(([title,sub,body])=><div className="comparison-cell" key={title}><small>{sub}</small><strong>{title}</strong><p>{body}</p></div>)}
  </div>;
  ...
  if (visual === 'security') return <div className="security-stack">
    ...
    ].map(([n,title,body])=><div className="security-layer" key={title}><span>{n}</span><strong>{title}</strong><p>{body}</p></div>)}
  </div>;
  ...
  if (visual === 'tracking') return <div className="usecase-table">
    ...
    ].map(([who,need,ops])=><div className="usecase-row" key={who}><strong>{who}</strong><span><small>Primary need</small>{need}</span><span><small>Operational reality</small>{ops}</span></div>)}
  </div>;
  ...
  if (visual === 'cameras') return <div className="coverage-grid">
    ...
    ].map(([n,title,body])=><div className="coverage-card" key={title}><span>{n}</span><h3>{title}</h3><p>{body}</p></div>)}
  </div>;
...
export default function DetailPage({ item, kind }) {
  ...
  return <main>
    <section className="detail-hero dark-section grid-bg"><div className="shell detail-hero__grid">
      <div><p className="eyebrow">{kind} / {item.eyebrow}</p><h1>{item.title}</h1><p className="lead lead--dark">{item.summary}</p><div className="button-row"><Link className="button" href="/start">{item.cta} →</Link><Link className="button button--ghost-dark" href="/resources/faq">Questions first</Link></div></div>
      <HeroPanel item={item} kind={kind}/>
    </div></section>
