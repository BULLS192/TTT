...
import { dealerApplicationResearch, tttDealerReadinessChecklist } from './dealerApplicationResearch';
import { portfolioCoverage } from './portfolioCoverage';
import { tttPartnerProfile, partnerApplicationNarratives } from './partnerApplicationNarratives';
...
import { vehicleFitmentIntelligence } from './vehicleFitmentIntelligence';
import { resolveVehicleFitment, getVehicleResearchGaps } from './fitmentResolver';
import { recurringPlans } from './recurringPlans';
import { laborClasses, quoteReadinessRequirements } from './laborModel';
import { packageCandidates } from './packageCandidates';
import { allCommercialSkus, createPlanningBom, createPlanningBomFromPackage, getCommercialDataGaps } from './commercialQuoteModel';
...
export const commercialCatalog = Object.freeze({
  ...
  vendorPipeline,
  dealerApplications:dealerApplicationResearch,
  dealerReadiness:tttDealerReadinessChecklist,
  partnerProfile:tttPartnerProfile,
  partnerNarratives:partnerApplicationNarratives,
  skus:allCommercialSkus,
  publicProductSkus:productSkus,
  securityProductSkus,
  vehicleSpecificSkus,
  vehicleFitment:vehicleFitmentIntelligence,
  recurringPlans,
  laborClasses,
  quoteReadinessRequirements,
  packageCandidates,
...
export function getCommercialSnapshot(){
  ...
  const gaps=getCommercialDataGaps();
  return Object.freeze({
    ...
    portfolioCoverageCount:portfolioCoverage.length,
    vendorChannelCount:vendorChannels.length,
    dealerApplicationResearchCount:dealerApplicationResearch.length,
    dealerReadinessOpenCount:tttDealerReadinessChecklist.filter((item)=>item.status==='needs-confirmation').length,
    skuCount:gaps.skuCount,
    skuWithPublicPrice:gaps.skuWithPublicPrice,
    vehicleFitmentPlatformCount:vehicleFitmentIntelligence.length,
    recurringPlanCount:recurringPlans.length,
    packageCandidateCount:packageCandidates.length,
    laborClassCount:laborClasses.length,
...
export {createPlanningBom,createPlanningBomFromPackage,getCommercialDataGaps,resolveVehicleFitment,getVehicleResearchGaps};
